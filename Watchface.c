...
static void main_window_load(Window *window) {
  ...
  //Creating background image layer for Pikachu
  s_background_bitmapPKMN = gbitmap_create_with_resource(RESOURCE_ID_PIKACHU);
  s_background_layerPKMN = bitmap_layer_create(GRect(73, 90, 62, 64));
  ...
  s_time_layer = text_layer_create(GRect(4, 52, 140, 50));
  text_layer_set_background_color(s_time_layer, GColorClear);
  text_layer_set_text_color(s_time_layer, GColorBlack);

  // Improve the layout to be more like a watchface
  s_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_MARKERS_36));
